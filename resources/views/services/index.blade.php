...
<div class="mb-6 flex justify-between items-center">
    <h3 class="text-2xl font-semibold text-gray-700">Data Services</h3>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-100 text-sm text-gray-500 uppercase tracking-wider">
                    ...
                    <th class="py-4 px-6 font-medium">Price (Rp)</th>
            ...
            <tbody class="divide-y divide-gray-100 text-sm text-gray-700">
                @forelse ($services as $index => $service)
                    <tr class="hover:bg-gray-50 transition duration-150">
                        ...
                        <td class="py-4 px-6">{{ number_format($service['price'], 0, ',', '.') }}</td>
                        ...
                        <td class="py-4 px-6 text-center relative">
                            <button onclick="toggleDropdown('dropdown-{{ $service['id'] }}', event)" class="dropdown-btn bg-gray-100 hover:bg-gray-200 text-gray-600 px-3 py-1.5 rounded-md text-sm transition">
                                Aksi ▾
                            </button>
                            <div id="dropdown-{{ $service['id'] }}" class="dropdown-menu hidden absolute right-6 top-12 mt-1 w-40 bg-white border border-gray-100 rounded-lg shadow-lg z-10 py-1 text-left">
                                <button onclick="openEditModal({{ json_encode($service) }})" class="w-full text-left px-4 py-2 text-blue-600 hover:bg-gray-50 transition">
                                    Edit Data
                                </button>
                                ...
                                <form action="{{ route('services.destroy', $service['id']) }}" method="POST" onsubmit="return confirm('Hapus service ini?')">
                                    @csrf @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="6" class="py-8 text-center text-gray-500">Belum ada data service.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-xl shadow-xl sm:my-8 sm:align-middle sm:max-w-lg w-full">
            <form action="{{ route('services.store') }}" method="POST">
                ...
                <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
                    <h3 class="text-lg font-semibold text-gray-900">Tambah Service</h3>
                ...
                <div class="px-6 py-4 space-y-4">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 outline-none" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Price (Rp)</label>
                        <input type="number" name="price" value="{{ old('price') }}" min="0" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 outline-none" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                        <textarea name="description" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 outline-none">{{ old('description') }}</textarea>
                    </div>
                    <div>
        ...
        <div class="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-xl shadow-xl sm:my-8 sm:align-middle sm:max-w-lg w-full">
            <form id="editForm" method="POST">
                ...
                <div class="px-6 py-4 space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Name</label>
                        <input type="text" name="name" id="edit_name" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 outline-none" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Price (Rp)</label>
                        <input type="number" name="price" id="edit_price" min="0" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 outline-none" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                        <textarea name="description" id="edit_description" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 outline-none"></textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Status</label>
